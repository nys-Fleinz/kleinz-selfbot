...
module.exports = {
    ...
    run: async (client, message, args, kleinz) => {
        ...
        if (args[0] === "load") {
            message.delete();
            if(!args[1]) return console.log(kleinz.console.info, "Please enter name of your config: spotify load configName".yellow)
            const filePath = path.join(directoryPath, `${args[1]}.json`);
            fs.access(filePath, fs.constants.F_OK, (err) => {
                if (err) {
                    return console.log(kleinz.console.X, `The file "${args[1]}.json" does not exist. ${err}`.red);
                } else {
                    try {
                        loadSpotifyCfg(filePath);
                    } catch (error) {
                        console.log(kleinz.console.X, "There was an error. Please check if you have set all the required settings and have a valid file name.".red);
                        console.log(error);
                    }
                }
            });
...
\u001b[0;30m[!] Type "spotify load [name]" to load\`\`\``)
            ...
            console.log(kleinz.console.success, `Loaded default file: "${settings.RPC.spotify_defaultFileName}.json"`.green);
            try {
                loadSpotifyCfg(filePath);
            } catch (error) {
                console.log(console.console.X, "There was an error. Please check if you have set all the required settings and have a valid file name.".red);
                console.log(error);
            }
        }
        ...
        function loadSpotifyCfg(filePath) {
            ...
            if(!spotifyCfg.IMAGES.largeImageID || !spotifyCfg.ALBUM.albumName || !spotifyCfg.ARTISTS.artistsName || !spotifyCfg.SONG.songName || !spotifyCfg.SONG_LENGHT.minutesSongDuration || !spotifyCfg.SONG_LENGHT.secondsSongDuration) return console.log(kleinz.console.info, "Check if you have filled all the required settings in your config.".yellow);
